...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Seller.js sends each variant's file separately and stores its
      // position in variant.imageIndex. Upload those files and replace the
      // temporary index with the permanent image object used by the buyer page.
      ...
      variantArr = variantArr.map((variant) => {
        const imageIndex = variant.imageIndex !== null &&
          variant.imageIndex !== undefined &&
          Number.isInteger(Number(variant.imageIndex))
          ? Number(variant.imageIndex)
          : null;
        const image =
          imageIndex !== null ? uploadedVariantImages[imageIndex] || undefined : undefined;
        return {
...
router.get("/:id", async (req, res) => {
  try {
    const p = await Product.findById(req.params.id).populate(
      "sellerId",
      "name storeName profilePhoto avatar city rating",
    );
